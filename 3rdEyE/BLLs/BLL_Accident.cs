...
{
    public class BLL_Accident : BaseBLL
    {
        ...
        public VM_Accident ConvertToViewModel(Accident model)
        {
            var viewModel = new VM_Accident();
            viewModel.Model = model;
            //# Relational property

            //# Only view property
            viewModel.IsDeleted_Text = model.IsDeleted == true ? "Deleted" : "Undeleted";
            viewModel.OccuranceDate_Text = model.OccuranceDate == null ? "" : CommonClass.ConvertToDateString(model.OccuranceDate);
            viewModel.Status_Text = model.Status == 0 ? "Open" : "Close";
