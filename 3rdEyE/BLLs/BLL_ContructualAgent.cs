...
{
    public class BLL_ContructualAgent : BaseBLL
    {
        public List<VM_AppUser> GetAllViewModels()
        {
            var list = db.AppUsers.AsEnumerable().Where(c => c.IsDeleted == false && (c.AppUserType == "ContructualAgent")).Select(c => ConvertToViewModel(c)).ToList();
            return list;
        }
        ...
        public AppUser FilterToDBModel(AppUser model)
        {
            ...
            db_model.AppUserType = "ContructualAgent";
            db_model.FK_Depo = model.FK_Depo;
            db_model.FK_ContructualRequisitionCompany = model.FK_ContructualRequisitionCompany;
